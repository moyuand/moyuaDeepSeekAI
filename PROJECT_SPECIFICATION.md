...
**功能描述**:
- 🆕 **刷新保持对话**: 页面刷新时保持当前对话内容，用户不会丢失正在进行的对话
- 🆕 **新对话按钮**: 提供便捷的入口发起新对话，清空当前消息

**核心场景**:
1. **意外刷新恢复**: 用户误刷新页面时，当前对话内容和上下文完整保留
2. **快速开始新话题**: 用户完成一个话题后，点击"新对话"按钮快速开始下一个话题
3. **切换对话便捷**: 在历史对话和当前对话之间无缝切换

**技术实现**:

**刷新保持对话**:
```javascript
// 方案1: 使用 localStorage 持久化当前会话（推荐）
...
**新对话按钮**:
- **位置方案A（推荐）**: 位于聊天头部（ChatHeader.vue），主题切换按钮旁边
- **位置方案B**: 历史记录侧边栏顶部（作为"新建对话"的主要入口）
- **图标**: 使用"编辑"或"加号"图标
- **快捷键**: Ctrl/Cmd + N
...
| 存储位置 | 存储内容 | 有效期 | 清除时机 |
|---------|---------|--------|---------|
| localStorage | 当前会话消息 | 24小时 | 1. 点击"新对话"<br>2. 切换到历史对话<br>3. 数据过期 |
| Pinia Store | 运行时消息状态 | 会话期间 | 页面关闭 |
| 后端数据库 | 已保存的历史对话 | 永久 | 用户手动删除 |
...
**边界情况处理**:
1. **多标签页同步**: 不同标签页的 localStorage 独立，避免冲突
2. **过期数据清理**: 自动清理超过24小时的持久化数据
3. **存储空间限制**: localStorage 最多存储 5MB，需控制消息数量
4. **无效会话处理**: 如果 sessionId 在后端不存在，创建新会话
...
**Header 组件按钮说明**:
- `[≡]` - 切换历史记录侧边栏/抽屉
- `[✏️]` - 🆕 **新对话按钮**：清空当前对话，开始新会话（快捷键 Ctrl/Cmd + N）
- `[🔆]` - 切换主题（Light/Dark）
- `[⚙]` - 跳转到设置页面
- `[↗]` - 跳转到关于页面（或其他外部链接）
...
- **位置**: 主题切换按钮左侧，设置按钮右侧
- **图标**: 编辑/笔记图标 (✏️)
- **样式**: quaternary 圆形按钮，与其他头部按钮一致
- **行为**:
  - 有未保存消息时：弹出确认对话框，避免误操作
  - 无未保存消息时：直接清空并聚焦输入框
  - 清空 localStorage 中的 currentSession 数据
  - 重置 Pinia Store 的 messages 和 currentSessionId
- **快捷键**: Ctrl/Cmd + N（全局监听）
...
- 🆕 **新对话**: 点击新对话按钮（✏️图标）→ 检查是否有未保存消息 → 二次确认（如需）→ 清空当前对话 → 清空 localStorage → 聚焦输入框
- 🆕 **刷新保持**: 页面刷新（F5/Ctrl+R）→ 从 localStorage 恢复当前会话 → 检查数据有效性 → 恢复消息和 sessionId → 继续对话
...
```javascript
{
  ...
  actions: {
    ...
    clearPersistedSession()    // 清除持久化的会话数据
  }
}
```
...
const restoreSession = () => {
  ...
  try {
    ...
    if (age < 24 * 60 * 60 * 1000) {
      currentSessionId.value = session.sessionId;
      messages.value = session.messages;
      logger.info('Session restored', { sessionId: session.sessionId });
      return true;
    } else {
      // 数据过期，清除
      localStorage.removeItem('currentSession');
